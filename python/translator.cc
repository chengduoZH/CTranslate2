...
template <typename T>
std::vector<T> py_list_to_std_vector_T(const py::object& l) {
  std::vector<T> v;
  v.reserve(py::len(l));
  for (const auto& s : l)
...
template <typename T>
static std::vector<std::vector<T>> batch_to_vector_T(const py::object& l,
                                                             bool optional = false) {
  ...
  for (const auto& handle : l) {
    if (handle.is(py::none())) {
      ...
      v.emplace_back(py_list_to_std_vector_T<T>(handle.cast<py::object>()));
  }
  return v;
}
...
public:
  ...
  py::list run_batch(const py::object& source) {
    if (source.is(py::none()) || py::len(source) == 0)
      return py::list();

    assert_model_is_ready();
    const auto source_input = batch_to_vector_T<size_t >(source);
...
PYBIND11_MODULE(translator, m)
{
  ...
  py::class_<TranslatorWrapper>(m, "Translator")
    ...
    .def(py::init<std::string, std::string, int, std::string, size_t, size_t>(),
            ...
            py::arg("inter_threads")=1,
            py::arg("intra_threads")=4)
    .def("run_batch", &BertWrapper::run_batch,
            py::arg("source"))
    ;
}
